...
subroutine init
  ...
  use var, only: t1, t2, &
       ...
       side_eq_UC, infile_qd, infile_qdd, &
       nq_chk, infile_q, fl_last
  implicit none
  integer :: i, j, k
  ...
  if ( (infile == '-h') .or. (infile == '') ) then
    write(*,'(a31)') ' Syntax: nd2xyz <setting file>'
    write(*,*)
    stop
  end if
  ...
  if ( trim(infile_q(:)) == '0' ) then
    write(*,'(*(a))') '  No ND trajectory file provided.'
  else
  ...
  if ( trim(infile_qd(:)) == '0' ) then
    write(*,'(*(a))') '  No ND velocity file provided.'
  else
    inquire(file=infile_qd,exist=file_exists)
    if ( file_exists .eqv. .false. ) then
      write(*,'(*(a))') ' ERROR: ND velocity file ',trim(infile_qd),' not found.'
      write(*,*)
      stop
    end if
    write(*,'(*(a))') '  Reading ND velocities from file: ', trim(infile_qd)
  end if

  read(10,*) infile_qdd
  if ( trim(infile_qdd(:)) == '0' ) then
    write(*,'(*(a))') '  No ND acceleration file provided.'
  else
  ...
  if ( (trim(infile_q(:)) == '0') .and. (trim(infile_qd(:)) == '0') .and. (trim(infile_qdd(:)) == '0') ) then
    write(*,'(a)') ' No input files provided.'
  ...
  read(10,*,iostat=i) fl_last

  if ( fl_last == 1 ) then
    write(*,'(*(a))') '  Write restart flag found: ignoring the time window and extracting the last configuration'
  end if
